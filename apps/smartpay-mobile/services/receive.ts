...
export async function generateReceiveQR(
  ...
  try {
    ...
    }>('/api/v1/user/profile', { retry: true });
  ...
  try {
    ...
    if (namqrResult.isValid && namqrResult.data) {
      ...
      try {
        ...
        }>(`/api/v1/users/lookup?smartpayId=${smartpayId}`, {
          retry: true,
        });
    ...
    if (extractedId) {
      ...
      try {
        ...
        }>(`/api/v1/users/lookup?smartpayId=${extractedId}`, {
          retry: true,
        });
  ...
  try {
    const expiresInHours =
      params.expiresInMinutes !== undefined
        ? Math.max(1, Math.ceil(params.expiresInMinutes / 60))
        : undefined;

    // Backend returns: { success: true, paymentRequest: { ... } }
    const response = await api.post<{
      ...
      '/api/v1/payment-requests',
      {
        ...
        note: params.note,
        // Backend expects `expiresIn` as hours.
        ...(expiresInHours !== undefined ? { expiresIn: expiresInHours } : {}),
      },
      { retry: false }
    );
