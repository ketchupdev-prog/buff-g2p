...
export async function getGroupById(groupId: string): Promise<Group | null> {
  try {
    const response = await api.get<{ data: Group }>(`/api/v1/groups/${groupId}`);
...
export async function createGroup(
  ...
  try {
    ...
    if (typeof nameOrParams === 'string') {
      ...
      // Extract member phones from request params
      phonesToInvite = Array.isArray(nameOrParams.memberPhones) ? nameOrParams.memberPhones : [];
    }
...
export async function joinGroup(groupId: string): Promise<{ success: boolean; error?: string }> {
  try {
    await api.post(`/api/v1/groups/${groupId}/join`, {}, { retry: false });
    return { success: true };
  } catch (error) {
...
export async function leaveGroup(
  ...
  try {
    if (!memberId) {
      throw new Error('Missing memberId for leaveGroup');
    }

    // Backend supports self-leave by removing the caller from group members:
...
export async function paySplit(
  ...
  try {
    ...
    const response = await api.post<PaySplitResponse>(
      `/api/v1/groups/${groupId}/splits/${splitId}/pay`,
      params,
      { retry: false }
    );
...
export async function remindSplit(
  ...
  try {
    await api.post(
      `/api/v1/groups/${groupId}/splits/${splitId}/remind`,
      {},
      { retry: false }
    );
