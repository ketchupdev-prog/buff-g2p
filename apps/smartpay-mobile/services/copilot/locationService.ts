 ...
 *
 * Backend (SmartPay): canonical nearest search is GET /api/v1/agents/nearest.
 * Mobile aliases (same handler, fixed type filter):
...
export interface AgentLocation {
  ...
  distance_km: number;
  /** Optional quality score when provided by the agents API */
  rating?: number;
}
...
export async function findNearbyAgents(
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/v1/agents/nearest?${params}`, {
      headers,
    });
...
export async function findNearbyATMs(
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/v1/atms/nearby?${params}`, {
      headers,
    });
...
export async function findNampostOffices(
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/v1/locations/nampost?${params}`, {
      headers,
    });
