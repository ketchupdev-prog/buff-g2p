 ...
 */
import { getSecureItem, getAccessTokenKey } from '@/services/secureStorage';
import { find_nearby_agents, find_nearby_atms, find_nampost_offices } from './copilot/copilotTools';
import { searchKnowledgeBase, getEducationalContent, getRelatedTopics } from '@/services/copilot/knowledgeBaseService';
import { createWallet, updateWallet, archiveWallet } from './copilot/walletManagementService';
...
export const copilotTools = {
  ...
  get_recent_activity: {
    ...
    description:
      'Summarise recent wallet activity (counts, amounts, top types). Uses the backend rolling summary window (fixed period on server; currently 30 days).',
    handler: async (_args: { days?: number }) => {
      const headers = await getAuthHeader();
      const res = await fetch(`${API_BASE}/api/v1/transactions/summary`, { headers });
  ...
  initiate_send_money: {
     ...
     * 
     * The `fromWalletId` is sent to the backend endpoint /api/v1/send-money.
     * 
    ...
    handler: async (input: SendMoneyInput) => {
      ...
      const res = await fetch(`${API_BASE}/api/v1/send-money`, {
        method: 'POST',
  ...
  initiate_cashout: {
    ...
    handler: async (input: CashoutInput) => {
      if (input.amount <= 0) throw new Error('Amount must be greater than zero.');
      const headers = await getAuthHeader();
      const url = `${API_BASE}/api/v1/cash-out/${input.method}`;
  ...
  redeem_voucher: {
    ...
    handler: async (input: { voucherId: string; method: 'wallet' | 'nampost' | 'smartpay'; walletId?: string }) => {
      const headers = await getAuthHeader();
      const base = `${API_BASE}/api/v1/vouchers/${input.voucherId}`;
      const path =
  ...
  get_proof_of_life_status: {
    ...
    handler: async () => {
      const headers = await getAuthHeader();
      const res = await fetch(`${API_BASE}/api/v1/user/profile`, { headers });
      if (!res.ok) throw new Error(await res.text());
  ...
  start_proof_of_life: {
    ...
    handler: async () => {
      const headers = await getAuthHeader();
      const res = await fetch(`${API_BASE}/api/v1/user/proof-of-life`, {
        method: 'POST',
        headers: { ...headers, 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      });
  ...
  get_loan_offer: {
    ...
    handler: async () => {
      const headers = await getAuthHeader();
      const res = await fetch(`${API_BASE}/api/v1/loans/eligibility`, { headers });
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
  },
  ...
  group_wallet_actions: {
    ...
    handler: async (input: { groupId: string; action: string; payload?: Record<string, unknown> }) => {
      ...
      if (action === 'balance') {
        const res = await fetch(`${API_BASE}/api/v1/groups/${groupId}/wallet`, { headers });
        if (!res.ok) throw new Error(await res.text());
        return res.json();
      }
      const res = await fetch(`${API_BASE}/api/v1/groups/${groupId}/${action}`, {
        method: 'POST',
        headers: { ...headers, 'Content-Type': 'application/json' },
  ...
  generate_cashout_qr: {
    ...
    handler: async (input: { walletId: string; amount: number; method: 'agent' | 'merchant' }) => {
      if (input.amount <= 0) throw new Error('Amount must be greater than zero.');
      const headers = await getAuthHeader();
      const res = await fetch(`${API_BASE}/api/v1/cash-out/qr`, {
        method: 'POST',
        headers: { ...headers, 'Content-Type': 'application/json' },
  ...
  create_wallet: {
    name: 'create_wallet',
    description: 'Create a new wallet with custom name, icon, and color. Guides user through wallet setup.',
    /**
  ...
  update_wallet: {
     ...
     * SECURITY NOTE: Wallet ownership verification
     * 
     * Updates wallet via PATCH /api/v1/wallets/:id
     * 
     * SERVER-SIDE VERIFICATION (backend/src/routes/mobile/wallets.ts):
     * 1. requireAuth middleware extracts userId from JWT token
     * 2. Database query enforces wallet ownership:
  ...
  get_wallet_overview: {
    ...
    handler: async () => {
      const headers = await getAuthHeader();
      const res = await fetch(`${API_BASE}/api/v1/wallets`, { headers });
      if (!res.ok) throw new Error(await res.text());
      const payload = await res.json();
      const wallets = Array.isArray(payload) ? payload : payload?.wallets;
      if (!Array.isArray(wallets)) {
        throw new Error('Invalid wallets response shape');
      }
  ...
  archive_wallet: {
     ...
     * SECURITY NOTE: Wallet ownership verification
     * 
     * Archives wallet via DELETE /api/v1/wallets/:id
     * 
