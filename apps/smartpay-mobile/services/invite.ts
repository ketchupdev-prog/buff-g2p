 ...
 * GET /api/v1/invite/validate?code=XXX
 */
...
export async function getMyReferralCode(): Promise<string | null> {
  try {
    const response = await api.get<{ referralCode: string }>('/api/v1/invite/me');
...
export async function getReferralStats(): Promise<ReferralStats | null> {
  try {
    const response = await api.get<ReferralStats>('/api/v1/invite/referrals');
    return response;
  } catch (error) {
