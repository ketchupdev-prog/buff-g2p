...
export async function initiateConsent(
  ...
  try {
    ...
    return result as AuthSession.AuthSessionResult;
  } catch (error) {
...
export async function disconnectBank(accountId: string): Promise<boolean> {
  try {
    ...
    // No SmartPay backend endpoint for local OAuth-linked accounts; OBS consents use
    // copilotTools.revoke_obs_consent → DELETE /api/v1/obs/consents/:id instead.
