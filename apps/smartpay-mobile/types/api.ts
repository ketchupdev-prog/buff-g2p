...
/**
 * Wallet type enum (internal backend classification only - not exposed to users)
 */
export type WalletType = 'main' | 'savings' | 'bills' | 'emergency' | 'travel' | 'shopping' | 'custom' | 'grant' | 'group' | 'general' | 'business';
...
export interface Wallet {
  ...
  balance: number;
  /** 
   * Internal type field for backend classification (not exposed to users).
   * Users define wallet purpose by naming it (e.g., "Groceries", "Trip Fund").
   * Defaults to 'custom' if not specified.
   */
  type?: 'main' | 'savings' | 'bills' | 'emergency' | 'travel' | 'shopping' | 'custom' | 'grant' | 'group' | 'general' | 'business';
  currency: string;
  ...
  isPrimary?: boolean;
  goalAmount?: number;
  /** Wallet tier level (basic, standard, premium) */
...
export interface CreateWalletRequest {
  name: string;
  /** 
   * Optional type field (internal use only).
   * Not exposed to users - they define wallet purpose by naming it.
   * Backend will default to 'custom' if not provided.
   */
  type?: Wallet['type'];
  icon: string;
  color: string;
...
/** GET /api/v1/transactions/summary — rolling window aggregates (backend: fixed 30 days). */
