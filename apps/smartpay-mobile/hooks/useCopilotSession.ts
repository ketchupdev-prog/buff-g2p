...
export function useCopilotSession(): UseCopilotSessionReturn {
  ...
  const { clearPendingAction, clearMessages } = useCopilotContext();
  ...
  const initializeSession = useCallback(async (): Promise<void> => {
    ...
    // Reset any copilot pending actions/messages when starting a new session.
    clearPendingAction();
    clearMessages();
    await fetchWalletData();
  }, [profile?.id, clearPendingAction, clearMessages, fetchWalletData]);
  ...
  }, [clearPendingAction, clearMessages]);
