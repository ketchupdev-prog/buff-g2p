...
const COPILOT_THREAD_STORAGE_KEY = 'smartpay_copilot_thread_id';
...
interface CopilotState {
  ...
  isSending: boolean;
  /** Server conversation thread id (persisted) */
  threadId: string;
  /** True after thread id is loaded from storage or created */
  threadReady: boolean;
}
...
interface CopilotContextValue extends CopilotState {
  ...
  /** Clear all messages */
  clearMessages: () => void;
  /** Start a new thread id and clear chat */
...
export function CopilotProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const stored = await AsyncStorage.getItem(COPILOT_THREAD_STORAGE_KEY);
        if (cancelled) return;
        if (stored?.trim()) {
          setThreadId(stored.trim());
        } else {
          const id = createThreadId();
          setThreadId(id);
          await AsyncStorage.setItem(COPILOT_THREAD_STORAGE_KEY, id);
        }
      } catch {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    AsyncStorage.setItem(COPILOT_THREAD_STORAGE_KEY, threadId).catch(() => {
      /* non-fatal */
    });
