...
function normalizeSmartpayId(raw: string): string {
  const value = String(raw ?? '').trim();
  if (!value) return 'SP-00000000';

  if (/^SP-\d{8}$/i.test(value)) return value.toUpperCase();
  if (/^SP\d{8}$/i.test(value)) return `SP-${value.slice(2)}`.toUpperCase();
...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const setSmartpayId = useMemo(
    () => (smartpayId: string) => {
      const normalized = normalizeSmartpayId(smartpayId);
      setProfile((prev) =>
        prev
          ? { ...prev, id: normalized }
          : { id: normalized, firstName: '', lastName: '' }
      );
    },
    []
  );

  const smartpayId = profile ? normalizeSmartpayId(profile.id) : undefined;
  const userName = profile ? `${profile.firstName} ${profile.lastName}`.trim() : undefined;
  const walletStatus: UserContextValue['walletStatus'] = profile
