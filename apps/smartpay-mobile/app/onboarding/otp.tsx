...
import { registerWithInviteCode } from '@/services/invite';
...
export default function OTPScreen() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // If the user arrived via an invite link, backend attribution is recorded here.
        if (!inviteAppliedRef.current) {
          inviteAppliedRef.current = true;
          try {
            const pendingInviteCode = await AsyncStorage.getItem(PENDING_INVITE_CODE_KEY);
            if (pendingInviteCode) {
              const reg = await registerWithInviteCode({
              ...
              if (reg.success) {
                ...
                console.warn('Invite attribution failed:', reg.error);
              }
            }
          } catch (e) {
            console.error('Invite attribution error:', e);
          }
        }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Prefer backend-driven cooldown if available; otherwise use a safe default.
        ...
        if (typeof result.retryAfterSeconds === 'number' && result.retryAfterSeconds > 0) {
          // Sync UI cooldown with backend rate-limiting window.
