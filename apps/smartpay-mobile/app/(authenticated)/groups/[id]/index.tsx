...
export default function GroupDetailScreen() {
  ...
  const leaveGroupMutation = useMutation({
    mutationFn: async () => {
      ...
      return leaveGroup(id, currentUserId);
    },
    onSuccess: (result) => {
      if (result.success) {
  ...
  const handleLeaveGroup = () => {
    if (!currentUserId) {
      Alert.alert('Error', 'Unable to identify your account for leaving this group');
      return;
    }
    const isAdmin = !!currentUserId && group?.members?.some((m) => m.userId === currentUserId && m.role === 'admin');
