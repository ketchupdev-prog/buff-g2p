...
const CASH_OUT_TYPE_CONFIG: Record<
  CashOutType,
  {
    title: string;
    instructions: string;
    fee: number;
    iconName: 'person' | 'business' | 'storefront';
    mockName: string;
  }
> = {
  till: {
    title: 'Cash Out at Till',
    instructions: "Scan the till's NAMQR code to proceed",
    fee: 0,
    iconName: 'storefront',
    mockName: 'Till #123',
  },
  agent: {
    ...
    instructions: "Scan the agent's NAMQR code to proceed",
    fee: 5,
    iconName: 'person',
    mockName: 'Agent John Doe',
  },
  merchant: {
    title: 'Cash Out at Merchant',
    instructions: "Scan the merchant's NAMQR code to proceed",
    fee: 0,
    iconName: 'business',
    mockName: 'Shop ABC',
  },
...
export default function CashOutTillScreen() {
  ...
  const typeConfig = CASH_OUT_TYPE_CONFIG[type];

  const handleScanQR = () => {
    // Simulate QR scan - in real app, this would open camera
    const mockData = {
      name: typeConfig.mockName,
      id: `${type}-${Date.now()}`,
    };
      ...
      {step === 'scan' && (
        <View style={styles.scanStep}>
          <View style={styles.instructionsCard}>
            ...
            <Text style={styles.instructionsText}>{typeConfig.instructions}</Text>
          </View>
