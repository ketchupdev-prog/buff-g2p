...
export function CopilotChatSurface() {
  ...
  const handleSend = useCallback(async () => {
    ...
    if (!text || isLoading || !threadReady || !threadId) return;
    setInput('');
    setError(null);
    ...
    try {
      ...
      const response = await sendChatMessage(text, threadId, token);
      ...
      } else if (response.status === 'interrupt') {
        appendMessage('assistant', 'I need your approval for this action.');
      }
    } catch (err) {
      const message =
        err instanceof Error ? err.message : 'Failed to get response from copilot';
      setError(message);
      appendMessage(
        'assistant',
        'I am having trouble reaching Copilot right now. Please try again in a few moments.'
      );
    } finally {
      setIsLoading(false);
      setIsSending(false);
  ...
  const handleSuggestion = useCallback(
    async (chip: SuggestionChip) => {
      if (isLoading || !threadReady || !threadId) return;
      setError(null);
      setInput('');
      ...
      try {
        const token = await getAuthToken();
        const response = await sendChatMessage(chip.prompt, threadId, token);
        ...
        appendMessage(
          'assistant',
          `I could not complete "${chip.label}" right now. Please try again.`
        );
      } finally {
        setIsLoading(false);
        setIsSending(false);
    ...
    [appendMessage, isLoading, threadId, threadReady, setIsSending]
  );
