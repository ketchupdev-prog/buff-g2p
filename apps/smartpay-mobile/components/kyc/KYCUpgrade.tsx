...
import { getAccessTokenKey, getSecureItem } from '@/services/secureStorage';
...
const Step3Documents: React.FC<any> = ({ formData, setFormData, walletType }) => {
  const pickDocument = async (documentType: string) => {
    try {
      ...
      // expo-document-picker returns `{ canceled: true }` or `{ canceled: false, assets: [...] }`
      if (!result.canceled) {
        const asset = result.assets?.[0];
        setFormData({ ...formData, [documentType]: asset ?? null });
      }
    } catch (error) {
