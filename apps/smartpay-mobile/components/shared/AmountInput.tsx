...
export function AmountInput({
  ...
  const renderKeyContent = (key: string) => {
    if (key === 'backspace') {
      return <Ionicons name="backspace-outline" size={28} color={DS.colors.text} />;
    }
    if (key === '') {
      return <View />;
    }
  ...
  // Ensure this is a strict boolean so it can be used safely in React Native style arrays.
  const isMaxReached =
    maxAmount !== undefined && parseFloat(amount) / 100 >= maxAmount;
