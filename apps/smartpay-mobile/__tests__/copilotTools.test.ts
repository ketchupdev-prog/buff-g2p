...
describe('Copilot Tools - Wallet & Balance', () => {
  ...
  describe('get_recent_activity', () => {
    it('should return transaction summary from GET /api/v1/transactions/summary', async () => {
      const mockSummary = {
        totalCredits: 5000,
        totalDebits: 3500,
        totalFees: 45.5,
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE}/api/v1/transactions/summary`,
        expect.any(Object)
      );
    });

    it('ignores tool `days` arg (backend summary window is fixed server-side)', async () => {
  ...
  describe('initiate_cashout', () => {
    ...
    it('should support all 5 cash-out methods', async () => {
      ...
      for (const method of methods) {
        ...
        expect(global.fetch).toHaveBeenCalledWith(
          `${API_BASE}/api/v1/cash-out/${method}`,
          expect.any(Object)
        );
      }
...
describe('Copilot Tools - Vouchers', () => {
  ...
  describe('redeem_voucher', () => {
    it('should redeem voucher to wallet', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE}/api/v1/vouchers/vch-123/redeem`,
        expect.any(Object)
      );
    });
    ...
    it('should redeem voucher to NamPost', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE}/api/v1/vouchers/vch-456/redeem-nampost`,
        expect.any(Object)
      );
    });
    ...
    it('should redeem voucher to SmartPay card', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE}/api/v1/vouchers/vch-789/redeem-smartpay`,
        expect.any(Object)
      );
    });
  });
...
describe('Copilot Tools - Group Wallets', () => {
  ...
  describe('group_wallet_actions', () => {
    it('should get group wallet balance', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE}/api/v1/groups/grp-123/wallet`,
        expect.any(Object)
      );
    });
    ...
    it('should contribute to group wallet', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE}/api/v1/groups/grp-123/contribute`,
    ...
    it('should support withdraw, send actions', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE}/api/v1/groups/grp-123/withdraw`,
        expect.any(Object)
      );
    });
  });
