...
# Test database URL (use test database or fallback to production DATABASE_URL)
...
async def clean_db(db_pool: asyncpg.Pool):
    ...
    async with db_pool.acquire() as conn:
        # Clear test data - only truncate tables that exist
        try:
            await conn.execute("TRUNCATE users, transactions, wallets CASCADE")
        except Exception:
            pass
    yield
    # Cleanup after test
...
async def sample_user(db_pool: asyncpg.Pool, clean_db) -> dict:
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow("""
            INSERT INTO users (first_name, last_name, full_name, phone, email, kyc_tier, created_at)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            RETURNING *
        """, "Test", "User", "Test User", "+26481234567", "test@example.com", "basic", datetime.now())
        return dict(row)
...
class TestQueryBuilder:
    ...
    async def test_simple_select(self, db_pool: asyncpg.Pool, sample_user: dict):
        ...
        assert results[0]["full_name"] == sample_user["full_name"]
    ...
    async def test_in_operator(self, db_pool: asyncpg.Pool, clean_db):
        ...
        async with db_pool.acquire() as conn:
            ...
            for i in range(3):
                row = await conn.fetchrow("""
                    INSERT INTO users (first_name, last_name, full_name, phone, kyc_tier, created_at)
                    VALUES ($1, $2, $3, $4, $5, $6)
                    RETURNING id
                """, f"User", f"{i}", f"User {i}", f"+26481234567{i}", "basic", datetime.now())
