...
# ---------------------------------------------------------------------------
# Agent Instance (Backward Compatible Export)
# ---------------------------------------------------------------------------

_transaction_analyst_instance: Optional[TransactionAnalystAgent] = None
...
# Backward-compatible export expected by `smartpay_ai.agents.transaction_analyst.__init__`
transaction_analyst_agent = get_transaction_analyst_agent().agent
...
async def run_transaction_analyst(
    ...
    """
    Backward-compatible runner for the Transaction Analyst agent.
    """
    ...
    return await agent.run(query=query, deps=deps, context=context)
