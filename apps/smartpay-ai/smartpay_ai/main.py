...
Purpose: Lifespan (DB pool + compile graph with checkpointer); mount versioned `/api/v1/*` routers.
"""
...
from smartpay_ai.api.analytics_endpoint import analytics_router
...
async def lifespan(app: FastAPI):
    ...
    # Initialize database pool (degraded mode allowed when DATABASE_URL is absent).
    try:
        await get_db_pool()
        logger.info("✓ Database pool created")
    except Exception as exc:
        logger.warning("⚠ Database pool unavailable; continuing in degraded mode: %s", exc)
    ...
    if ml_enabled:
        try:
            ...
            if ML_AVAILABLE:
                ...
                logger.warning("⚠ ML models not available (dependencies not installed)")
...
app = FastAPI(
    ...
    description=(
        "AI-powered financial assistant for Namibia's digital payment platform. "
        "HTTP APIs are versioned under `/api/v1/*` (copilot, ml, admin, analytics, health)."
    ),
    version="0.1.0",
    lifespan=lifespan,
    openapi_tags=[
        {"name": "copilot", "description": "Copilot chat (`/api/v1/copilot`)."},
        {"name": "copilot-streaming", "description": "SSE streaming chat (`/api/v1/copilot`)."},
        {"name": "health", "description": "Service health (`/api/v1/health`)."},
        {"name": "ml", "description": "ML predictions and training (`/api/v1/ml`)."},
        {"name": "admin", "description": "Administration (`/api/v1/admin`)."},
        {"name": "analytics", "description": "DuckDB analytics and reporting (`/api/v1/analytics`)."},
    ],
)
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "ml": {
                "predict": "POST /api/v1/ml/predict",
                "models": "GET /api/v1/ml/models",
                "health": "GET /api/v1/ml/health",
                "train": "POST /api/v1/ml/train",
            },
            "admin": {
                "ingest": "POST /api/v1/admin/knowledge-base/ingest",
                "stats": "GET /api/v1/admin/stats",
                "reload_models": "POST /api/v1/admin/models/reload",
                "system_info": "GET /api/v1/admin/system-info",
            },
            "analytics": {
                "transactions": "GET /api/v1/analytics/transactions",
                "users": "GET /api/v1/analytics/users/{user_id}",
                "fraud": "GET /api/v1/analytics/fraud",
                "reports_dashboard": "GET /api/v1/analytics/reports/dashboard",
