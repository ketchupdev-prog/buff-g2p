...
from smartpay_ai.config.transaction_limits import (
    KYCTier,
    ViolationSeverity,
    EMONEY_LIMITS as CONFIG_EMONEY_LIMITS,
    FIA_STR_THRESHOLD,
    FIA_CTR_THRESHOLD,
    get_limits_for_tier,
...
# PSD-1/PSD-3 limits (test-friendly shape)
# Note: the authoritative limits are `CONFIG_EMONEY_LIMITS` (dataclasses).
EMONEY_LIMITS: Dict[KYCTier, Dict[str, float]] = {
    tier: {
        "max_single_transaction": limits.max_single_transaction,
        "max_daily_transaction": limits.max_daily_transaction,
        "max_monthly_transaction": limits.max_monthly_transaction,
    }
    for tier, limits in CONFIG_EMONEY_LIMITS.items()
}

# PSD-11 card interchange rates (used by tests + documentation)
# Note: the authoritative fee logic lives in `smartpay_ai.shared.fee_calculator`.
INTERCHANGE_RATES: Dict[str, float] = {
    "debit": 0.005,
    "hybrid": 0.0075,
    "credit": 0.0155,
}
...
class RiskLevel(str, Enum):
    """Risk level classification used across fraud/compliance tooling."""
    LOW = "low"
    MEDIUM = "medium"
    HIGH = "high"
    CRITICAL = "critical"
...
def assess_risk_level(risk_score: float) -> str:
    """
    Convert an ML risk score (0.0-1.0) to a qualitative risk level string.

    Thresholds (kept consistent with existing monitoring logic in this module):
    - critical: score >= 0.90
    - high:     score >= 0.70
    - medium:   score >= 0.50
    - low:      score <  0.50
...
class SharedComplianceValidator:
    """
    Convenience wrapper around the shared validator functions.

    This class is intentionally thin: it centralizes configuration (backend URL, optional DB pool)
    and tracks basic fallback stats for monitoring.
    """
    ...
    def __init__(
        ...
        self.node_backend_url = node_backend_url or os.getenv("NODE_BACKEND_URL", "http://localhost:3000")
        self.db_connection_string = db_connection_string
        self.enable_db_fallback = enable_db_fallback
    ...
    async def initialize(self) -> None:
        """Initialize optional DB pool (only if enabled + connection string provided)."""
        if not self.enable_db_fallback or not self.db_connection_string:
            self.db_pool = None
            return
        self.db_pool = await asyncpg.create_pool(dsn=self.db_connection_string, min_size=1, max_size=5)
    ...
    async def validate_transaction_limits(
        ...
        """Validate limits using API with fallback to local validation."""
        ...
        result = await validate_transaction_limits(
            ...
            mode=ValidationMode.API_WITH_FALLBACK,
        )
        if str(result.get("source")) == ValidationSource.PYTHON_FALLBACK.value:
    ...
    async def estimate_interchange_fee(
        ...
        """Estimate interchange fee using API with fallback to local calculator."""
        self._total_api_calls += 1
        result = await estimate_interchange_fee(
    ...
    def check_fia_threshold(self, amount: float) -> Dict[str, Any]:
        """FIA helper (delegates to config implementation)."""
        return config_check_fia_threshold(amount)

    def get_fallback_stats(self) -> Dict[str, Any]:
        """Return basic stats for monitoring fallback behavior."""
        ...
        return {
            ...
            "db_fallback_enabled": bool(self.enable_db_fallback),
        }
...
def validate_emoney_limits(
    ...
    """
    Backward-compatible validator used by unit tests and legacy call sites.

    Returns:
        (is_valid, error_message, remaining_limits)
    """
    # Normalize tier
    try:
        tier_enum = KYCTier(user_tier)
    except Exception:
        return False, f"Invalid KYC tier: {user_tier}", {}

    limits = EMONEY_LIMITS[tier_enum]
    remaining_daily = limits["max_daily_transaction"] - daily_spent
    remaining_monthly = limits["max_monthly_transaction"] - monthly_spent
    remaining = {
        "daily": max(0.0, remaining_daily - amount),
        "monthly": max(0.0, remaining_monthly - amount),
    }
    ...
    if amount > limits["max_single_transaction"]:
        return (
            False,
            f"Transaction exceeds {tier_enum.value} tier single transaction limit of N${limits['max_single_transaction']:.2f} (attempted N${amount:.2f})",
            remaining,
        )
    ...
    if daily_spent + amount > limits["max_daily_transaction"]:
        return (
            False,
            f"Daily limit exceeded. Remaining daily limit: N${max(0.0, remaining_daily):.2f}",
            {"daily": max(0.0, remaining_daily), "monthly": max(0.0, remaining_monthly)},
        )
    ...
    if monthly_spent + amount > limits["max_monthly_transaction"]:
        return (
            False,
            f"Monthly limit exceeded. Remaining monthly limit: N${max(0.0, remaining_monthly):.2f}",
            {"daily": max(0.0, remaining_daily), "monthly": max(0.0, remaining_monthly)},
        )
...
def estimate_interchange_fee_local(
    ...
    result = legacy_calc.estimate_fee_local(transaction_type, card_type, amount)
    # Align legacy expectations: round to cents and present as a python fallback.
    if "vat_amount" in result:
        result["vat_amount"] = round(float(result["vat_amount"]), 2)
    if "interchange_amount" in result:
        result["interchange_amount"] = round(float(result["interchange_amount"]), 2)
    if "total_fee" in result:
        result["total_fee"] = round(float(result["total_fee"]), 2)
    # Ensure description contains expected legacy phrases (tests assert lowercase substrings).
    desc = str(result.get("description", ""))
    if transaction_type == "card_retail" and card_type:
        hint = f"{str(card_type).lower()} retail purchase"
        if hint not in desc.lower():
            desc = f"{hint} - {desc}"
