...
class TokenBucket:
    ...
    def _refill(self):
        ...
        # Avoid tiny floating-point drift impacting exact-token tests.
        if tokens_to_add < 1e-6:
            return
        self.tokens = min(self.capacity, self.tokens + tokens_to_add)
        # Snap extremely close values to exact integers.
        snapped = round(self.tokens, 6)
        if abs(self.tokens - snapped) < 1e-6:
            self.tokens = snapped
...
class RateLimitConfig:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path is None:
            ...
            # Go up to project root, then to packages/shared-config
            # current_dir = apps/smartpay-ai/smartpay_ai/shared
            # project_root should resolve to `fintech/`
            project_root = current_dir.parent.parent.parent.parent
            config_path = project_root / "packages" / "shared-config" / "rate_limits.yaml"
    ...
    def should_skip_path(self, path: str) -> bool:
        ...
        skip_paths = self.config.get("skip_paths") or []
