...
class DuckDBManager:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        # Initialize analytics modules with the same connection
        # This ensures in-memory databases are shared
        self.spending = SpendingAnalytics(db_path, conn=self.conn)
        self.fraud = FraudAnalytics(db_path, conn=self.conn)
        self.groups = GroupAnalytics(db_path, conn=self.conn)
    ...
    def _init_unified_schema(self):
        ...
        # Spending analytics tables
        self.conn.execute("""
            CREATE TABLE IF NOT EXISTS user_spending_patterns (
                user_id VARCHAR PRIMARY KEY,
                avg_daily_spending DECIMAL(10,2),
                avg_transaction_amount DECIMAL(10,2),
                most_frequent_category VARCHAR,
                most_frequent_merchant VARCHAR,
                last_updated TIMESTAMP
            )
        """)
        ...
        logger.info("Unified schema initialized with all tables")
