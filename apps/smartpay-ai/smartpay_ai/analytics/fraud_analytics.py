...
class FraudAnalytics:
    ...
    def __init__(self, db_path: str = ":memory:", conn: Optional[duckdb.DuckDBPyConnection] = None):
        """
    ...
    def load_fraud_events(self, df: pd.DataFrame):
        """
        Load fraud events from pandas DataFrame
        
        Args:
            df: DataFrame with fraud event columns
        """
        self.conn.execute("DELETE FROM fraud_events")
        self.conn.register('fraud_events_df', df)
        
        # Select columns explicitly to match schema
        self.conn.execute("""
            INSERT INTO fraud_events 
            (transaction_id, is_fraud, risk_score, flagged_reason, 
             timestamp, reviewed, reviewed_by, reviewed_at)
            SELECT 
                transaction_id, is_fraud, risk_score, flagged_reason,
                timestamp,
                ...
                reviewed_by,
                reviewed_at
            FROM fraud_events_df
        """)
        logger.info(f"Loaded {len(df)} fraud events from DataFrame")
