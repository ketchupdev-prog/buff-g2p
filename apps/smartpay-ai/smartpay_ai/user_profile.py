...
Purpose: DRY – profile lives in Node; Copilot calls GET /api/v1/user/profile with
...
async def fetch_user_profile(auth_token: str) -> Optional[Dict[str, Any]]:
    ...
    Returns the same user object the mobile app gets from GET /api/v1/user/profile.
    ...
    if not token:
        return None
    url = f"{SMARTPAY_API_BASE_URL}/api/v1/user/profile"
    try:
