...
router = APIRouter(prefix="/api/v1/copilot", tags=["copilot-streaming"])
...
async def chat_stream(
    ...
    deps_ctx = replace(deps, thread_id=req.thread_id)

    async def event_generator():
        """Wrap stream generator for EventSourceResponse."""
        async for event in _stream_generator(graph, req, config, deps_ctx):
            yield event
