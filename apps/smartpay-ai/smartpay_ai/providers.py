...
def get_llm_model(model_name: Optional[str] = None, **kwargs: Any) -> Any:
    ...
    # In tests (or when no API key is configured), return a deterministic in-process model
    # so the unit test suite doesn't depend on external providers.
    if os.getenv("PYTEST_CURRENT_TEST") or os.getenv("NODE_ENV") == "test" or not api_key:
        from pydantic_ai.models.test import TestModel
        return TestModel(custom_output_text="Smartpay Copilot test response")
