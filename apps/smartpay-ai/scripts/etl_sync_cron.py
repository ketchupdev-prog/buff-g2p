...
async def main():
    """Main ETL sync execution with data quality checks"""
    parser = argparse.ArgumentParser(description="DuckDB ETL Sync Cron Job with Quality Checks")
    parser.add_argument(
        "--sync-type",
    ...
    try:
        if args.skip_quality_checks:
            # Legacy mode: Run without quality checks
            logger.warning("Running sync without quality checks (--skip-quality-checks)")
            ...
            # New mode: Run with quality checks (recommended)
            service = ETLSyncService(config["pg_conn_string"])
            try:
                results = await service.sync_with_quality_checks(
                    sync_type=args.sync_type,
                    days_back=args.days_back
                )
            finally:
        ...
        if success_count == total_tables and quality_score >= 90:
            logger.info(f"✓ ETL sync completed successfully (quality: {quality_score}%)")
            sys.exit(0)
        elif success_count == total_tables:
            logger.warning(f"⚠ ETL sync completed with quality issues (quality: {quality_score}%)")
            sys.exit(1)
        else:
