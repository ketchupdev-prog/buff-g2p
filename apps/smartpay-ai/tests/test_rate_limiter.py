...
class TestConfigurableRateLimitMiddleware:
    ...
    def mock_request(self):
        ...
        request.url = Mock()
        request.url.path = "/api/v1/copilot/chat"
        request.state = Mock()
...
""")
            f.flush()  # Ensure data is written to disk before yielding
            temp_path = f.name
    ...
    async def test_per_user_limiting(self, config_file):
        ...
        request1.url = Mock()
        request1.url.path = "/api/v1/copilot/chat"
        request1.state = Mock()
        ...
        request2.url = Mock()
        request2.url.path = "/api/v1/copilot/chat"
        request2.state = Mock()
