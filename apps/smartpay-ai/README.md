...
### HTTP API (`/api/v1/*`)
#### Chat & Streaming
- `POST /api/v1/copilot/chat` - Standard chat endpoint (LangGraph + HITL; **`thread_id` required**)
- `POST /api/v1/copilot/chat/stream` - SSE streaming with real-time updates

##### `POST /api/v1/copilot/chat` (contract)

**Authentication:** `Authorization: Bearer <JWT>` — the handler resolves the user profile from the **Node API** (`SMARTPAY_API_BASE_URL`) for DRY context.

**Request body** (`ChatRequest` in [`smartpay_ai/agents/copilot/models.py`](./smartpay_ai/agents/copilot/models.py)):

| Field | Type | Required | Notes |
|-------|------|----------|--------|
| `thread_id` | string | **Yes** | LangGraph checkpointer key; reuse for one conversation, change for a new thread. |
| `message` | string | If `resume` unset | User message. Omit when resuming after HITL. |
| `user_id` | string | No | Optional; profile id normally comes from the JWT / Node profile. |
| `resume` | bool or object | No | When set, invokes `Command(resume=…)` for human-in-the-loop continuation. |

**Success response** (`ChatResponse`):

| Field | Notes |
|-------|--------|
| `status` | `"ok"` or `"interrupt"` (pending approval / HITL). |
| `messages` | List of `{ role, content }` when `status` is `ok`. |
| `approval_payload` | Present when `status` is `interrupt`. |
| `last_tool_result` | Optional string from the last tool execution. |
| `thread_id` | Echoes the request thread id. |

**Environment:** set **`LLM_API_KEY`** (and `LLM_PROVIDER` / `LLM_MODEL` as needed) in `.env` so the orchestrator can call the configured LLM.

**March 2026 fixes (reliability):**

- **`await` on `graph.ainvoke`** — async path returns the real graph result.
- **Thread-scoped execution** — `configurable.thread_id` in LangGraph matches the client conversation.
- **Upstream HTTP** — profile fetch and audit logging awaited end-to-end.
- **Postgres / Neon blips** — one retry on transient checkpointer connection/SSL errors before failing.
...
**Typical mobile path:** `POST http://localhost:4000/api/v1/copilot/chat` (Node proxy) with the same body and token — see [Copilot API guide](../../docs/guides/api/copilot-api.md). **Guardrails / scenarios:** [copilot-test-scenarios.md](../../docs/guides/reference/copilot-test-scenarios.md).

#### Health Monitoring
- `GET /health` - Basic health check (load balancer)
- `GET /api/v1/health/detailed` - Component health with latencies

#### ML Service
- `POST /api/v1/ml/predict` - Direct ML predictions
- `GET /api/v1/ml/models` - List available models
- `GET /api/v1/ml/health` - ML service status
- `POST /api/v1/ml/train` - Async training jobs

#### Admin (Protected)
- `POST /api/v1/admin/knowledge-base/ingest` - Bulk knowledge base ingestion
- `GET /api/v1/admin/stats` - Usage statistics
- `POST /api/v1/admin/models/reload` - Hot reload ML models
- `GET /api/v1/admin/system-info` - System diagnostics

#### Analytics (DuckDB)
- Base path: `/api/v1/analytics` — transactions, users, fraud, reports, ETL, and custom read-only queries. See OpenAPI at `/docs` or `smartpay_ai/analytics/README.md`.
...
# Smartpay Node.js API (same port as apps/smartpay-backend)
...
async def fetch_user_profile(auth_token: str) -> Optional[Dict]:
    url = f"{SMARTPAY_API_BASE_URL}/api/v1/user/profile"  # mobile/proof-of-life alias; also `GET /api/v1/users/profile`
    # ... fetch from Node API
```
...
# Test streaming
curl -N -X POST http://localhost:8000/api/v1/copilot/chat/stream \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
...
**Last updated:** 2026-03-21 (Copilot `/chat` schemas, `LLM_API_KEY`, March 2026 reliability fixes)
