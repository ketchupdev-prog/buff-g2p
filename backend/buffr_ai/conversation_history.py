"""
AI Companion conversation history management with user isolation.

Location: backend/buffr_ai/conversation_history.py
...
async def get_user_preferences(user_id: str) -> Dict[str, Any]:
    """
    Fetch user's AI Companion preferences.
...
async def update_user_preferences(user_id: str, **preferences) -> None:
    """
    Update user's AI Companion preferences.
...
async def get_conversation_stats(user_id: str) -> Dict[str, Any]:
    """
    Get aggregated statistics for user's AI interactions.
    """
    pool = await get_db_pool()
    ...
    row = await pool.fetchrow(
        ...
        SELECT 
            ...
            unique_threads, support_interactions, advice_interactions,
            avg_response_time_ms, total_tokens_used, avg_user_rating,
            flagged_count, first_interaction, last_interaction,
            messages_per_day_last_7d
    ...
    return {
        ...
        "support_interactions": row["support_interactions"],
        "advice_interactions": row["advice_interactions"],
        ...
        "avg_rating": float(row["avg_user_rating"]) if row["avg_user_rating"] else 0,
        ...
        "messages_per_day": float(row["messages_per_day_last_7d"]) if row["messages_per_day_last_7d"] else 0,
    }


async def should_summarize_conversation(user_id: str, thread_id: Optional[str] = None) -> Tuple[bool, int]:
    """
    Check if conversation should be summarized based on message count and user preferences.
    
    Returns:
        (should_summarize: bool, message_count: int)
    """
    pool = await get_db_pool()
    
    # Get user's auto-summarize threshold
    prefs = await get_user_preferences(user_id)
    threshold = prefs["auto_summarize_after_messages"]
    
    # Count messages in thread/conversation
    conditions = ["user_id = $1"]
    params: List[Any] = [UUID(user_id) if isinstance(user_id, str) else user_id]
    
    if thread_id:
        conditions.append("thread_id = $2")
        params.append(thread_id)
    
    where_clause = " AND ".join(conditions)
    
    row = await pool.fetchrow(
        f"SELECT COUNT(*) as count FROM ai_conversation_history WHERE {where_clause}",
        *params,
    )
    
    message_count = row["count"]
    return (message_count >= threshold, message_count)


async def cleanup_old_conversations(user_id: Optional[str] = None) -> int:
    """
    Delete old conversations based on retention preferences.
    If user_id is provided, only cleanup for that user.
    Otherwise, cleanup for all users (call from cron).
    ...
    if user_id:
        ...
        prefs = await get_user_preferences(user_id)
        if not prefs["store_conversation"]:
            # User disabled storage - delete all
            result = await pool.execute(
                "DELETE FROM ai_conversation_history WHERE user_id = $1",
                UUID(user_id) if isinstance(user_id, str) else user_id,
            )
        else:
            # Delete based on retention days
            cutoff = datetime.now() - timedelta(days=prefs["conversation_retention_days"])
            result = await pool.execute(
                """
        ...
        result = await pool.execute("SELECT cleanup_old_conversations()")
    ...
    logger.info(f"Cleaned up {count} old conversation messages for user_id={user_id or 'all'}")
    return count
...
async def format_conversation_for_llm(
    ...
    *,
    ...
    Format:
        ...
        Assistant: To send money with Buffr...
