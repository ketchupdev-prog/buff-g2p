...
Paths assume the repo sits next to **Buffr Connect** and **Ketchup Portals** under the same parent directory:

| Workspace | Path | Role |
|-----------|------|------|
| **Buffr AIS (primary API)** | `../buffr-connect/buffr-ais-platform/` | OIDC + AIS + consent APIs; **`@buffr/connect-sdk` `baseUrl`** should target this deploy. |
| **Buffr portal (optional)** | `../buffr-connect/buffrconnect/` | Large TPP/UI app—skip for **AIS-only** tracks; see repo root `ARCHITECTURE_CORRECTIONS_2026-03-22.md`. |
| **Buffr bank sims (dev)** | `../buffr-connect/banks/*` | Local ASPSP OAuth/AIS sandboxes—not production deploy targets. |
| **Ketchup Portals** | `../ketchup-smartpay/ketchup-portals/` | G2P web portals; Buffr API env for voucher flows |

**Last reviewed:** 2026-03-21

**Next ecosystem actions (deploy, smoke tests, backend test triage):** see `fintech/TASKS.md` → **Ecosystem: deployment & verification (2026-03-22)**.

### Environment files (ecosystem map)

**Do not commit** real `.env`, `.env.local`, or `.env.production.local`. Canonical templates are `*.env.example` / `.env.local.example` / `.env.test.example` per app. **Never paste secrets into docs or chat.**

**Load order (Next.js apps, e.g. Ketchup / Buffr web):** `.env` → `.env.local` → `.env.development.local` (etc.); later files override. Use **one variable name once per file** (duplicate keys are confusing; last assignment wins).

**Directory tree (where env files live — not an exhaustive repo-wide scan):**

```text
ai-agent-mastery-main/
├── fintech/apps/smartpay-backend/
│   ├── .env.example          # template (copy → .env)
│   ├── .env / .env.local     # local only (gitignored)
├── ketchup-smartpay/ketchup-portals/
│   ├── .env.example
│   ├── .env.test.example
│   ├── .env / .env.local / .env.test
│   └── backend/.env.example  # legacy sibling API (if used)
├── ketchup-smartpay/backend/
│   ├── .env.example
│   └── .env.local            # Ketchup BFF/API keys (gitignored)
├── buffr-connect/buffrconnect/
│   ├── .env.example, .env.local.example, .env.production.example, .env.ussd.example
│   └── .env / .env.local / .env.test
└── buffr-connect/buffr-ais-platform/
    ├── .env.example, .env.production.example
    └── .env.local
```

**Canonical cross-repo variable names and ports** (Buffr origin vs `BUFFR_API_URL`, webhooks, SmartPay `4000`): [`ketchup-smartpay/ketchup-portals/docs/DOMAIN_AND_ENV_RECOMMENDATIONS.md`](../ketchup-smartpay/ketchup-portals/docs/DOMAIN_AND_ENV_RECOMMENDATIONS.md) and [`DNS_AND_REDIRECTS.md`](../ketchup-smartpay/ketchup-portals/docs/DNS_AND_REDIRECTS.md).

**Cross-app alignment (typical):**

| Concern | Ketchup Portals | SmartPay backend | Buffr Connect |
|--------|-----------------|------------------|---------------|
| Supabase | `NEXT_PUBLIC_SUPABASE_*`, `SUPABASE_SERVICE_ROLE_KEY` | `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` | Same pattern + `NEXT_PUBLIC_*` |
| SmartPay API | `SMARTPAY_BACKEND_URL` / `NEXT_PUBLIC_SMARTPAY_BACKEND_URL` → backend **:4000** | `PORT=4000`, `DATABASE_URL`, `JWT_*` | `SMARTPAY_WEBHOOK_SECRET` (when integrated) |
| Buffr Open Banking | `BUFFR_API_URL`, `BUFFR_API_KEY`, `BUFFR_WEBHOOK_SECRET` | `BUFFR_CONNECT_URL` (origin), `BUFFR_API_URL` (…/api), `BUFFR_WEBHOOK_SECRET` | Buffr Connect `WEBHOOK_SECRET` = same **value** as `BUFFR_WEBHOOK_SECRET` when paired; AIS split: see AIS `NEXT_PUBLIC_APP_URL` / SDK base |
| SMS / Twilio | — | `TWILIO_*`, `ALLOW_DEV_FALLBACK` | — |
| BoN reporting | — | `BON_API_BASE_URL`, `BON_API_KEY`, optional `BON_API_URL` | — |

**Hygiene / pitfalls resolved or to avoid:**

- **Nested junk paths:** Ignore duplicated templates under `buffr-connect/buffrconnect/ketchup-smartpay/` or `buffr-connect/fintech/` — use the real app roots in the tree above only.
- **SmartPay backend `BON_*`:** `.env.example` documents a single `BON_API_KEY`; `BON_API_BASE_URL` (reporting-client) vs `BON_API_URL` (incidentReporter) are both listed with comments.
- **Ketchup `ketchup-smartpay/backend`:** `.env*.bak` backups removed from repo; `.gitignore` extended so new backup copies are not committed.
- **Buffr Connect `.env.local`:** duplicate `NEXT_PUBLIC_POSTHOG_*` keys were deduped (last line kept per key).

**Verify locally (keys only, no values):**

```bash
for d in ketchup-smartpay/ketchup-portals fintech/apps/smartpay-backend \
         buffr-connect/buffrconnect buffr-connect/buffr-ais-platform; do
  echo "== $d =="; find "$d" -maxdepth 1 -name '.env*' -type f 2>/dev/null | sort
done
```

#### Neon MCP checks (SmartPay Postgres project)

Using the **Neon MCP** (`user-neon` in Cursor) against the org **smartpay**:

| Check | Result |
|--------|--------|
| Project | **`smartpay`** — id `hidden-tree-34889452`, Postgres **17**, region `aws-us-east-1` |
| Default branch | **`production`** — id `br-holy-sea-amymtymc`, state **ready** |
| SQL connectivity | `SELECT current_database()` → database **`neondb`**, role **`neondb_owner`** |
| Schema | **100+** `public` relations; includes **`vouchers`**, **`voucher_redemptions`**, agents, compliance, ML, OBS, etc. |

**Not a Neon outage:** If Vitest shows `fetch failed` / `UNABLE_TO_GET_ISSUER_CERT_LOCALLY` with `@neondatabase/serverless`, that is **local Node/undici TLS trust**, not the cloud DB. **Ketchup Portals** uses the `pg` driver when `VITEST=true` (`src/lib/db.ts`) with SSL **on** for remote hosts and **off** for `localhost` / `sslmode=disable`.

**Vitest `DATABASE_URL`:** Copy the **full** connection string from the Neon dashboard (user is usually **`neondb_owner`**, not `postgres`). A bad URL causes `role "postgres" does not exist` or auth errors. For local Docker Postgres, use a real role/password and keep host as `localhost` so SSL stays disabled.
...
├── CONSOLIDATION_COMPLETE.md  # Security consolidation record (PSD-12)
...
**Note (Security - PSD-12):** Canonical implementation lives in `smartpay/backend/src/security/`. The former `fintech/security/` duplicate folder has been removed to enforce DRY.
...
## Recent PostgreSQL migrations (incremental)

- **`database/migrations/043_user_notifications.sql`** — `notifications` inbox (FK `users`, JSONB `metadata`, indexes for list + unread). Backend access: `apps/smartpay-backend/src/lib/userNotificationsRepository.ts` (DRY SQL); routes in `routes/mobile/notifications.ts`.
- **`database/migrations/044_vouchers_portal_columns.sql`** — extends `vouchers` with `voucher_code`, `voucher_type`, `issuer`, `issued_at`, `redemption_method_allowed`, `redeemed_at`, `updated_at`, `metadata` (IF NOT EXISTS) + unique index on `voucher_code`. Required for mobile redemption routes and **`voucher.issued`** Buffr webhook persistence. Apply alone: `npm run migrate:vouchers-portal --workspace=@smartpay/backend` (or `MIGRATION_ONLY=044_vouchers_portal_columns.sql` with the shared migrate script).
- **Runner:** `npm run migrate` (from `fintech/`) executes `apps/smartpay-backend/scripts/runMigrations.ts` against **`fintech/database/migrations/*.sql`** (sorted). Loads `apps/smartpay-backend/.env` for `DATABASE_URL`.
- **Existing DBs** whose `migrations` table does not match that folder (e.g. provisioned with an older runner) may hit “already exists” on early files. Options: (1) baseline the `migrations` table to match what is already applied, or (2) run a single file: `MIGRATION_ONLY=043_user_notifications.sql npm run migrate --workspace=@smartpay/backend`, or (3) `npm run migrate:notifications --workspace=@smartpay/backend`.

### SmartPay Mobile — integration tests (real DB / API)

- **Default unit tests:** `npm test --workspace=@smartpay/mobile` **excludes** `__tests__/integration/real-*` and `__tests__/integration/setup/` (helpers are not test files; mocked flows such as `copilot-flows.test.ts` still run).
- **Integration:** `npm run test:integration --workspace=@smartpay/mobile` — runs **`__tests__/integration/real-*.test.ts`** against **`TEST_DATABASE_URL`** (or `DATABASE_URL`) with `pg`, and a running SmartPay backend (default `http://localhost:4000`, override with `TEST_BACKEND_PORT`).
- **Buffr webhook:** set **`BUFFR_WEBHOOK_SECRET`** to the same value on the backend and in the test env. Backend route: `POST /api/buffr/webhooks` with `X-Buffr-Event-Type: voucher.issued` — handler: `apps/smartpay-backend/src/routes/buffr-webhooks.ts` (`handleVoucherIssued`).
- **Apply schema:** run migration **044** (or full migrate) on the DB used for integration tests so `vouchers` has portal columns.

---

**Last Updated:** 2026-03-18 14:30 UTC  
**Status:** Phase 8 ACTIVE - Monorepo Migration Execution (98% system health)

---

## 🚀 ACTIVE PHASE: Monorepo Migration Execution

**Start Date:** 2026-03-18 14:30 UTC  
...
**Primary Goals:**
1. ✅ Execute monorepo restructure following MONOREPO_MIGRATION_PLAN.md
2. ✅ Preserve 100% of git history
3. ✅ Maintain zero breaking changes
4. ✅ Achieve clean separation: apps/ | packages/ | docs/
5. ✅ Centralize shared code and eliminate duplication

**Success Criteria:**
- [ ] All apps build successfully in new structure
- [ ] All tests pass (313/313 maintained)
- [ ] Import paths updated and verified
- [ ] Documentation reorganized and indexed
- [ ] Shared packages created and linked
- [ ] Zero functionality lost
- [ ] Git history fully preserved

### Multi-Agent Execution Plan

**Agent Team (5 specialized agents):**

1. **Agent M1 - Apps Migration Specialist** (30-45 min)
   - Move smartpay/backend → apps/smartpay-backend
   - Move smartpay/mobile → apps/smartpay-mobile  
   - Move smartpay/backend_python → apps/smartpay-ai
   - Preserve git history with `git mv`
   - Update package.json names

2. **Agent M2 - Packages Creation Specialist** (45-60 min)
   - Create packages/shared-types from JSON schemas
   - Create packages/shared-config from configs
   - Create packages/shared-security (placeholder)
   - Set up package.json for each
   - Generate TypeScript/Python types

3. **Agent M3 - Import Path Updater** (60-90 min)
   - Scan all TypeScript files (108 files)
   - Update imports: types/generated → @smartpay/shared-types
   - Update relative paths for new structure
   - Verify TypeScript compilation
   - Update Python imports (minimal changes)

4. **Agent M4 - Documentation Reorganizer** (30-45 min)
   - Move compliance docs → docs/compliance/BON_PSDs/
   - Move playbooks → docs/playbooks/
   - Organize guides by category
   - Create docs/README.md index
   - Update all markdown links

5. **Agent M5 - Configuration & Verification** (30-45 min)
...
- **Group 1 (Parallel):** Agent M1 + Agent M4 (no dependencies)
- **Group 2 (Depends on M1):** Agent M2 + Agent M3 (after apps moved)
- **Group 3 (Final):** Agent M5 (after all changes complete)

**Rollback Plan:**
- Backup created before migration: `fintech-backup-YYYYMMDD-HHMMSS.tar.gz`
- Git branch: `feature/monorepo-migration`
- One-command rollback: `./scripts/migrate-to-monorepo.sh --rollback`
