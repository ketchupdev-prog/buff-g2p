...
export interface Transaction {
  ...
  /** Optional display date (alias for createdAt when present from API). */
  date?: string;
  ...
  if (API_BASE_URL) {
    try {
      ...
      if (res.ok) {
        ...
        if (data && typeof data === 'object' && 'transaction' in data) return (data as { transaction?: Transaction }).transaction ?? null;
